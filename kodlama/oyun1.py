...
import time
import locale
from karakterler import Bullet, Player
...
def oyun():
    ...
    p1 = Player(int(WIDTH/2),HEIGHT-20,RED)
    # p2 = Player(250,100,30,40,3,SILVER)
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                if event.key==pygame.K_SPACE:
                    b1=Bullet(p1.rect.centerx-10,p1.rect.top,YELLOW)
                    b2=Bullet(p1.rect.centerx+10,p1.rect.top,TEAL)
                    all_sprites.add(b1)
                    all_sprites.add(b2)
